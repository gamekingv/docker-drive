...
function iconFormat(originalName: string): string {
  ...
  if (name === 'combine') return 'mdi-image-multiple-outline';
  if (/.*\.(mp4|mkv|avi)$/.test(name)) return 'mdi-video-outline';
  if (/.*\.(ass|srt|ssa|vtt)$/.test(name)) return 'mdi-closed-caption-outline';
  if (/\.(jpe?g|png|gif|bmp|webp|ico)$/.test(name)) return 'mdi-file-image-outline';
  if (/\.(mp3|ogg|wav|flac|aac)$/.test(name)) return 'mdi-music-box-outline';
  if (/\.(m3u8)$/.test(name)) return 'mdi-playlist-music';
  return 'mdi-file';
}
...
export { sizeFormat, formatTime, progressPercentage, iconFormat, iconColor };
