  ...
  async ({
    ...
    switch (type) {
      case 'count': {
        ...
        const info: { name: string; digest: string; range?: number[] }[] = [];
        (function getCount(items: FileItem[], path: string): void {
          for (const item of items) {
            if (item.type === 'file') {
              fileCount++;
              fileSize +=
                (item.start || item.start === 0) && item.end
                  ? item.end - item.start + 1
                  : (item.size as number);
              const range =
                (item.start || item.start === 0) && item.end ? [item.start, item.end] : undefined;
              info.push({
                name: `${path}${item.name.replace(/[\\/*?:<>|"]/g, '')}`,
          ...
          for (const file of rootFiles.files as FileItem[]) {
            if (file.type === 'folder') {
              const filterFile = {
                name: file.name,
                files: [],
                id: `${path}/${encodeURIComponent(file.name)}`,
                disabled: files.some(e => e.id === file.id)
              };
