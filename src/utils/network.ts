...
export default {
    ...
    try {
      ...
      if (status === 401) {
        try {
          ...
          const { status, headers } = error.response ?? {};
          if (status === 401)
            throw { message: 'need login', authenticateHeader: headers['www-authenticate'] };
          else throw error;
        }
      }
      throw error;
  ...
  async getToken(authenticateHeader: string, repository: Repository): Promise<string | undefined> {
    ...
    const [, realm, service, , scope] =
      authenticateHeader?.match(/^Bearer realm="([^"]*)",service="([^"]*)"(,scope="([^"]*)"|)/) ??
      [];
    if (realm && service) {
  ...
  async getDownloadURL(digest: string, repository: Repository): Promise<string> {
    if (buildAsExtension) {
      ...
      const getHeader = (
        e: chrome.webRequest.WebResponseHeadersDetails
      ): void | chrome.webRequest.WebResponseHeadersDetails => {
        if (e.statusCode >= 300 && e.statusCode < 400) {
          downloadURL = e.responseHeaders?.find(e => e.name.toLowerCase() === 'location')
            ?.value as string;
          request.cancel('ok');
      ...
      try {
        chrome.webRequest.onHeadersReceived.addListener(getHeader, { urls: [url] }, [
          'blocking',
          'responseHeaders'
        ]);
      ...
      const { data: downloadUrl } = await axios.get(
        `/api/file/${digest.replace('sha256:', '')}?repo=${repository.id}`
      );
      return downloadUrl;
    }
  },
  async downloadFile(digest: string, repository: Repository): Promise<AxiosResponse> {
    if (buildAsExtension) {
      ...
      return await axios.get(
        `/api/file/${digest.replace('sha256:', '')}?type=download&repo=${repository.id}`
      );
  },
  async sentToAria2(
    items: { name: string; digest: string; range?: number[] }[],
    repository: Repository
    ...
    items.forEach((item, i) =>
      requestBody.push({
        ...
        params: [
          ...
          [
            buildAsExtension
              ? `https://${server}/v2/${namespace}/${image}/blobs/${item.digest}`
              : `${protocol}//${host}/api/file/${item.digest.replace(
                  'sha256:',
                  ''
                )}?type=source&repo=${repository.id}`
          ],
          {
            out: `${item.name}`,
            header: buildAsExtension
              ? [
                  `repository: ${repository.url}`,
                  `Authorization: Bearer ${token}`,
                  item.range ? `Range: bytes=${item.range[0]}-${item.range[1]}` : undefined
                ]
              : [item.range ? `Range: bytes=${item.range[0]}-${item.range[1]}` : undefined]
          }
        ]
  ...
  async uploadFile(
    ...
    const [{ headers }, digest] = await Promise.all([
      this.requestSender(`${url}`, patchInstance, repository),
      this.hashFile(file, hashWorker)
    ]);
    const instance = axios.create({
      method: 'put',
      headers: {
